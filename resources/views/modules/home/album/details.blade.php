...
<div class="under-hero container">
        ...
        <div class="row align-items-center">
            ...
            <div class="col-md-8 mt-5 mt-md-0">
                <div class="d-flex flex-wrap mb-2">
                    <span class="text-dark fs-4 fw-semi-bold pe-2 capitalize">{{ $album->name }}</span>
                </div>
                ...
                <p>Desctiption: {{ $album->description }}</p>
                <ul class="info-list">
                    <li>
                        <div class="d-flex align-items-center">
                            <button type="button" id="play_all" class="btn btn-icon btn-primary rounded-pill"
                                data-collection-play-id="{{ $album->id }}" data-type="album" data-id="{{ $album->id }}"
    ...
    <div class="section">
        <div class="section__head">
            <div class="flex-grow-1">
                <h3 class="mb-0">All <span class="text-primary">Songs</span></h3>
                <span style="display:block;" class="section__subtitle">of -
                    {{ $album->name }} -</span>
            </div>
        </div>
        <div class="list list-album-songs" style="display: grid;
                       grid-template-columns: repeat(2, 1fr);
                       column-gap: 20px;row-gap: 0px;" data-collection-song-id="{{ $album->id }}">
            @if ($songs->isEmpty())
            <p class="empty">No Song Available</p>
            @else
            @foreach ($songs->take(20) as $item)
            @php
            ...
            <div class="list__item" data-song-id="{{ $item->id }}" data-song-name="{{ $item->name }}"
                ...
                data-song-album="{{ $album->name }}" data-song-url="{{ asset('music/' . $item->url ) }}"
                data-song-cover="{{ $img }}">
                    ...
                    <li class="dropstart d-inline-flex">
                        ...
                        <ul class="dropdown-menu dropdown-menu-sm">
                            ...
                            <li class="item-playlist">
                                <a class="dropdown-item" href="javascript:void(0);" role="button"
                                    data-song-id-item="{{ $item->id }}">Add to
                                    Playlist</a>
                                <div class="sub-menu-2">
                                    <span class="create"><a data-change="false" data-song-id-item={{$item->id}}
                            ...
                            <li>
                                <a class="dropdown-item" href="{{ route('home.download-song', ['id' => $item->id]) }}"
                                    role="button">Download</a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="{{ route('home.song-details', ['id' => $item->id]) }}"
        ...
        @if ($album->songs->count() > 20)
        <div class="mt-5 text-center">
            <a href="javascript:void(0);" data-type="album" data-limit="20" data-quantity="20"
                data-id="{{ $album->id }}" data-load="song" class="btn btn-primary btn-load-more">
                <div class="btn__wrap">
    ...
    <div class="section">
        <div class="section__head">
            <h3 class="mb-0">
                Related <span class="text-primary">Albums</span>
            </h3>
        </div>
        <div class="swiper-carousel swiper-carousel-button">
            <div class="swiper" data-swiper-slides="5" data-swiper-autoplay="true">
                <div class="swiper-wrapper">
                    ...
                    @else
                    @foreach ($otherAlbums as $item)
                    @php
                    $img = $item->img_url == null || !file_exists(public_path('img/album/' . $item->img_url)) ?
                    $item->img_url : asset('img/album/' . $item->img_url);
                    $favorite = App\Models\Favorite::where('album_id', $item->id)->where('user_id',$user ->id)->first();
                    @endphp
                        ...
                        <div class="cover cover--round">
                            <div class="cover__head">
                                <ul class="cover__label d-flex">
                                    <li class="icon-fvr">
                                        <label class="add-fvr" data-type="album" data-favorite-id="{{ $item->id }}"
                                            for="album[{{ $item->id }}]">
                                            <input {{ $favorite ? 'checked' : '' }} title="like" type="checkbox"
                                                class="like" id="album[{{ $item->id }}]">
                                            <div class="checkmark">
                                ...
                                <div class="cover__options dropstart d-inline-flex ms-auto">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-sm">
                                        <li>
                                            <a class="dropdown-item" href="javascript:void(0);" role="button"
                                            data-collection-play-id="{{ $item->id }}" 
                                            data-type="album" 
                                            data-id="{{ $item->id }}">Play</a>
                                        </li>
                                        <li>

                                            <a class="dropdown-item"
                                                href="{{ route('home.album-details', ['id' => $item->id]) }}">View
                                                details</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                            <a href="{{ route('home.album-details', ['id' => $item->id]) }}" class="cover__image"><img
                                    src="{{ $img }}" alt="{{ $item->name }}" /></a>
                            <div class="cover__foot">
                                <a href="{{ route('home.album-details', ['id' => $item->id]) }}"
                                    class="cover__title text-truncate">{{ $item->name }}</a>
                            </div>
                        </div>
                    </div>
                    @endforeach
